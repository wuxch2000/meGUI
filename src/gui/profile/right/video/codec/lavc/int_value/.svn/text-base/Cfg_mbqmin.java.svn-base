...
public class Cfg_mbqmin extends A_SimpleCfgInt {

    private final static String label = "mbqmin";
    private final static String xmlPath = "/profile/video/codec/lavc/" + label;
    private final static String tip = "minimum macroblock quantizer (pass 1/:2) ";
    private final static int defaultValue = 2;
    private final static int minValue = 1;
    private final static int maxValue = 31;

    public Cfg_mbqmin() {
        super(label, xmlPath,
            FactoryDefaultBehaviour.build(NO_OUTPUT), FactoryArgOutputFormat.build(EQUAL), defaultValue, minValue,
            maxValue);
